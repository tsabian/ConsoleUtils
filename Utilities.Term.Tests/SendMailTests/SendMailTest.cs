    ...
    public class SendMailTest
    {
        ...
        public async Task SendMailSuccessTest()
        {
            var args = new string[] { 
                ...
                "tsabian@hotmail.com", 
                "-p", 
                SimpleTemplateTestFilePath,
                "-s", 
                "Teste de envio de e-mail",
                "-r", 
                SimpleReplacementTestFilePath
            };
            var sendMailCommand = new SendMailCommand(args);
            Assert.Equal(CommandsStatus.Started, sendMailCommand.Status);
            await sendMailCommand.Execute();
            Assert.Equal(CommandsStatus.Successfully, sendMailCommand.Status);
         }
    }
}